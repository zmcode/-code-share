      ...
      InfiniteLoopController: {
        ...
        _calcLoop(loopID) {
          if (this._loopMap.has(loopID)) {
            let { isInit, sumExeTime, startTime, count } =
              this._getLoop(loopID);
            if (isInit) {
        ...
        _loopMonitor(loopID) {
          ...
          if (loop.sumExeTime > maxSumExeTime && loop.count > maxLoopCount) {
            this._clearLoops();
            throw new Error(
              "This loop executes so many times that JS-Encoder have to exit the loop in case the page gets stuck"
            );
          }
        },
      },
