<template>
  <div id="user" class="d-flex flex-clo flex-ai">
    <div
      class="user-info-box flex-clo flex-ai"
      v-show="!userInfoLoading"
      :style="{ '--user-bgc': `url(${qiNiuImgLink + curUserDetail.avatar})` }"
    >
      <v-avatar
        size="100"
        class="avatar"
        :color="curUserDetail.avatar ? '' : 'primary'"
      >
        <v-img
          v-if="curUserDetail.avatar"
          :src="qiNiuImgLink + curUserDetail.avatar"
        ></v-img>
        <span class="white--text text-h4" v-else>{{
          curUserDetail.nickname | preNickname
        }}</span>
        ...
        <v-btn
          ...
          fab
          x-small
          v-else-if="loginState && !isMyFollow"
          :loading="followLoading"
          @click="follow"
        >
      ...
      <div class="info-text d-flex flex-1 flex-clo flex-ai">
        <span class="nickname text-bold title-md">{{
          curUserDetail.nickname
        }}</span>
        ...
        <v-col md="2" sm="8" cols="12">
          ...
          >
            ...
            >
              ...
              <div class="item-num text-xs">
                {{ num[item.path] | formatNumber }}
              </div>
            </v-tab>
          </v-tabs>
        </v-col>
...
export default {
  ...
  methods: {
    ...
    async getUserInfo() {
      ...
      try {
        const { data } = await this.$http.getUserInfo({ userId: id });
        if (data) {
          const {
            userName: nickname,
            avatar,
