      ...
      <v-card-text>
        ...
        <div class="d-flex flex-clo">
          ...
          <span class="text-xs text-describe"
            >你所添加的外部样式，将按照顺序在本地CSS执行之前依次执行，支持http和https协议链接。</span
          >
          <v-autocomplete
            flat
            dense
          ...
          >
            <template v-slot:item="{ item }">
              <v-list-item
                ...
                link
                @click="addLink('css', item)"
              >
                <span style="margin-right: 20px" class="flex-1">{{
                  item.name
                }}</span>
          ...
          <v-text-field
            class="lib-input-list"
            solo
            dense
            flat
            background-color="info"
            hide-details
            v-model="cssUseList[index]"
            v-for="(item, index) in showCSSInput"
            :key="`css${index}`"
          >
            <template v-slot:prepend-inner>
              <div class="pointer d-flex flex-clo" @click.prevent>
                <v-icon
                  ...
                  small
                  :disabled="index === 0"
                  @click="resort(index, 'up', 'css')"
                  >mdi-chevron-up
                </v-icon>
                <v-icon
                  class="icon"
                  small
                  :disabled="index === showCSSInput - 1"
                  @click="resort(index, 'down', 'css')"
                >
                  mdi-chevron-down
                </v-icon>
              </div>
          ...
          <v-btn class="add-btn" block color="info" @click="showCSSInput++"
            >添加外部样式</v-btn
          >
          <span class="text-md lib-title">外部脚本</span>
          <span class="text-xs text-describe"
            >你所添加的外部脚本，将按照顺序在本地JavaScript执行之前依次执行，支持http和https协议链接</span
          >
          <v-autocomplete
            :items="jsLibList"
            flat
          ...
          <v-text-field
            ...
            background-color="info"
            hide-details
            v-for="(item, index) in showJSInput"
            :key="`js${index}`"
            v-model="jsUseList[index]"
          >
            <template v-slot:prepend-inner>
              <div class="pointer d-flex flex-clo" @click.prevent>
                <v-icon
                  ...
                  small
                  :disabled="index === 0"
                  @click="resort(index, 'up', 'js')"
                  >mdi-chevron-up
                </v-icon>
                <v-icon
                  class="icon"
                  small
                  :disabled="index === showJSInput - 1"
                  @click="resort(index, 'down', 'js')"
                >
                  mdi-chevron-down
                </v-icon>
              </div>
          ...
          <v-btn class="add-btn" block color="info" @click="showJSInput++"
            >添加外部脚本</v-btn
          >
        </div>
      </v-card-text>
      ...
      this.$http
        ...
        .then(({ results }) => {
          try {
            ...
            this.jsLibList = Object.freeze(
              results.filter((item) => item.fileType === "js")
            );
